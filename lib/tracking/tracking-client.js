...
/**
 * Get track likes and check if current user has liked
 * @param {string} accessToken - User's access token (optional)
 * @param {string} trackId - Track ID
 * @returns {Promise<Object>} Likes info or error
 */
export const getTrackLikes = async (accessToken, trackId) => {
  try {
    const headers = {};
    if (accessToken) {
      headers.Authorization = `Bearer ${accessToken}`;
    ...
    const response = await fetch(`${API_BASE}/${trackId}/likes`, { headers });
    const data = await response.json();
    ...
    return { error: { message: 'Failed to fetch track likes' } };
  }
};

...
import { generateStaticMap } from '../map/static-map-generator.js';
...
/**
 * Generate map image for the trip
...
const generateMapImage = async (data) => {
  ...
  try {
    const mapBuffer = await generateStaticMap({
      points,
    ...
    console.warn('Error generating map image:', error.message);
    return null;
  }
};
...
export const generateTripPdf = async (trip) => {
  ...
  // Static Map
  const mapBuffer = await generateMapImage(data);
  if (mapBuffer) {
    try {
      ...
      // Embed the PNG image
      const mapImage = await pdfDoc.embedPng(mapBuffer);
      ...
      const legendText =
        data.stops.length > 0
          ? `S = Start | Numbers = Day stops (${data.stops.length} park${data.stops.length > 1 ? 's' : ''})`
          : 'Trip route map';

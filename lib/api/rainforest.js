 ...
 */
const getAmazonTag = () => process.env.AMAZON_TAG || 'parklookup-20';
...
/**
 * Make a request to the Rainforest API with retry on server errors
...
const makeRequest = async (params, retryCount = 0) => {
  ...
  if (!response.ok) {
    ...
    if (isServerError(response.status) && retryCount < 1) {
      console.warn(
        `⚠️ Rainforest API server error (${response.status}), cooling down for 30 seconds...`
      );

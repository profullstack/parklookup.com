...
/** Wikidata entity IDs for park types */
export const PARK_TYPES = {
  NATIONAL_PARK: 'Q46169', // national park in the United States
  STATE_PARK: 'Q15243209', // state park in the United States
};

/**
 * Builds a SPARQL query to fetch U.S. State Parks from Wikidata
 * (National Parks are imported separately from the NPS API)
 *
 * Uses name-based filtering (" State " in label) because the Wikidata
 * entity Q15243209 is too broad and includes many non-park entities.
 * The space around "State" ensures we match "State Park", "State Beach",
 * "State Recreation Area", etc. but not "Interstate".
 *
 * @param {Object} options - Query options
 * @param {number} [options.limit=50] - Number of results per page
 * @param {number} [options.offset=0] - Pagination offset
 * @param {string} [options.nameFilter=' State '] - Filter parks by name containing this string
...
WHERE {
  # Get all parks (Q22698 = park) and their subclasses in the USA
  ?park wdt:P31/wdt:P279* wd:Q22698.
  ?park wdt:P17 wd:Q30.
  
  # Filter by name containing "${nameFilter}"
  ?park rdfs:label ?parkLabel.
  FILTER(LANG(?parkLabel) = "en")
  FILTER(REGEX(?parkLabel, "${nameFilter}", "i"))
...
export const fetchWikidataParkById = async (wikidataId) => {
  const query = `
    ...
    WHERE {
      ...
      # Verify it's a park (Q22698) or subclass
      ?park wdt:P31/wdt:P279* wd:Q22698.

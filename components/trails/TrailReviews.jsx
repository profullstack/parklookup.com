...
export default function TrailReviews({ trailId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch comments
        const commentsRes = await fetch(`/api/trails/${trailId}/comments`, {
          headers: authHeaders,
        });
        if (commentsRes.ok) {
        ...
        // Fetch likes status
        const likesRes = await fetch(`/api/trails/${trailId}/likes`, {
          headers: authHeaders,
        });
        if (likesRes.ok) {
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/trails/${trailId}/likes`, {
        method,
  ...
  const handleCommentSubmit = async (content, rating) => {
    ...
    try {
      ...
      const res = await fetch(`/api/trails/${trailId}/comments`, {
        method: 'POST',
        headers: {
  ...
  const handleCommentEdit = async (commentId, content, rating) => {
    try {
      const res = await fetch(`/api/trails/${trailId}/comments/${commentId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...getAuthHeaders(),
